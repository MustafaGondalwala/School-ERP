...
export class ManageClerkLogin extends Component {
  constructor(props) {
    super(props);
    var date = new Date();
    ...
    this.state = {
      ...
      add_button_text: "Add",
      add_view_button_text: "Add",
      view_add_form: false,
      staff_info: "",
      view_individual_clerk: "",
      update_button_text:"Update"
    };
    this.addNewClerkUser = this.addNewClerkUser.bind(this);
    this.addFormViewHide = this.addFormViewHide.bind(this);
    this.updateClerkInfo = this.updateClerkInfo.bind(this);
  }

  addNewClerkUser(data) {
    var self = this;
    this.setState({
      add_button_text: "Adding Clerk ...",
    });
    axios({
      url: "/api/v1/fee/manage_login",
      method: "post",
      data: data,
    })
      .then((response) => {
        self.setState(
          ...
          () => {
            Swal.fire("Clerk Added!", response.data.success.message, "success");
          }
        );
      })
      .catch((errors) => {
          ...
          () => {
            if (errors.response.status == 422)
              Swal.fire(
                ...
                "Validation Error Occured while Add the Clerk",
                "error"
              );
            else
              Swal.fire(
                "Errror Occured",
                "Error Occur while Add the Clerk",
                "error"
              );
          }
        );
  ...
  addFormViewHide(type) {
    if (type == false)
      this.setState({
        view_add_form: true,
        add_view_button_text: "Hide Add Form",
      	edit_individual_clerk:"",
        	view_individual_clerk:""
      });
    else
      this.setState(
  ...
  componentDidMount() {
    ...
    axios({
      ...
      self.setState({
        staff_info: response.data.success.staff_info,
      });
    });
  }
  editClerk(clerk_id){
       this.addFormViewHide(true);
       	this.setState({
		    edit_individual_clerk:clerk_id,
		    view_individual_clerk:""
		})
  ...
  viewClerk(id) {
    this.addFormViewHide(true);
    if (this.state.view_individual_clerk == id) {
      this.setState({
        view_individual_clerk: "",
        edit_individual_clerk:""
      });
      ...
      this.setState({
        view_individual_clerk: id,
        edit_individual_clerk:""
      });
    }
  }
  updateClerkInfo(data){
    var self = this;
  	Swal.fire({
      title: 'Are you sure?',
      text: "All the Clerk Details will be Updated!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, Update Clerk Info!'
    }).then((result) => {
      if (result.value) {

	    self.setState({
	      update_button_text: "Updating Clerk ...",
	    });
	    axios({
	      url: "/api/v1/fee/manage_login",
	      method: "put",
    ...
    const columns = [
      ...
      },
      {
        ...
        cell: (row) => (
          <div>
            <button
              onClick={(e) => this.editClerk(row.id)}
      ...
      },
      {
        ...
        cell: (row) => (
          <div>
            <button
              onClick={(e) => this.viewClerk(row.id)}
              className="btn btn-sm btn-primary"
            >
              View
            </button>
    ...
    return (
      <div>
        ...
        <div className="container-fluid mt--6">
              ...
              <div className="card-header border-0">
                <div className="row">
                  ...
                  <div className="col-6 text-right">
                    <button
                      onClick={(e) => this.addFormViewHide(view_add_form)}
          ...
          {view_individual_clerk && (
            <ViewIndividualClerk  submit={this.updateClerkInfo} clerk_id={view_individual_clerk} />
          )}

          {edit_individual_clerk &&
          	<UpdateIndividualClerk submit={this.updateClerkInfo} update_button_text={this.state.update_button_text} data={data} clerk_id={edit_individual_clerk}/>
          }
        </div>
      </div>
    );
            ...
            <div className="col-lg-6">
              <div className="card">
                <div className="card-body">
                  ...
                  {this.state.updateinstallment_message && (
                    <div
                  ...
                  <SetInstallmentsForm
                    add_text_message={this.state.add_text_message}
                    submit={this.updateFeesInstallment}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="container-fluid mt--6">
          <div className="card mb-4">
            ...
            <div className="card-body">
              ...
              <SetDueDateForm
                add_text_button={this.state.add_text_button}
                select_year={this.state.select_year}
                submit={this.updateDueDate}
              />
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="container-fluid mt--6">
          <SelectIndividualStudent
            ...
            add_student_button_text_individual={
              this.state.add_student_button_text_individual
            }
            submit={this.fetchStudent}
          />
          {this.state.receipts && (
            <ShowReceiptList
              Receipts={this.state.receipts}
              receiptClick={this.receiptClick}
              view_receipt_button_text={this.state.view_receipt_button_text}
              print_receipt_button_text={this.state.print_receipt_button_text}
            />
          )}
          {this.state.receiptDetails && (
            <ShowReceipt
              ...
              selectedReceiptId={this.state.selectedReceiptId}
            />
          )}
        </div>
      </div>
...
export const ShowReceipt = ({
  ...
  <div className="card mb-4">
    ...
    <div className="card-body">
      {studentDetail && (
        <div className="row">
          <div className="col-md-4">
            <label>Roll No: {studentDetail.roll_no}</label>
          </div>
          <div className="col-md-4">
      ...
      <br />
      {receiptDetails &&
        Object.keys(receiptDetails).map((item) => {
          return (
            <div className="row">
              <div className="col-md-4">
                <h3>{item}</h3>
              ...
              <div className="col-md-12">
                <div className="table-responsive">
                  <table className="table">
                    ...
                    {receiptDetails &&
                      Object.keys(receiptDetails[item]).map((feeItem, id) => {
                        if (
                          feeItem != "total_amount" &&
                          feeItem != "total_pending" &&
                          feeItem != "current_paid"
                        )
                          return (
                            <tr>
                              <td>{id + 1}</td>
                              <td>
                                {receiptDetails[item][feeItem].fee.fees_type}
                              </td>
                              <td>
                                {
                                  receiptDetails[item][feeItem].fee
                                    .after_discount
                                }
                              </td>
                              <td>
                              ...
                              <td>
                                {
                                  receiptDetails[item][feeItem].reciept
                                    .current_paid
                                }
                              </td>
                            </tr>
...
export const ShowReceiptList = ({
  Receipts,
  receiptClick,
  view_receipt_button_text,
  print_receipt_button_text,
}) => (
  <div className="card mb-4">
    <div className="card-header">
              ...
              Receipts.map((item, id) => {
                return (
                  <tr>
                    ...
                    <td>
                      <button
                        data-receipt-id={item.reciept}
                        data-action={"view"}
                        onClick={(e) => receiptClick(e)}
      ...
      <div>
        ...
        <div className="container-fluid mt--6">
          ...
          {this.state.total_fees_type &&
            Object.keys(this.state.total_fees_type).map((item) => {
              return (
                <InstallmentUpdate
                  type="pay_fees"
                  onChange={this.onChange}
                  installment={item}
                  total_fees_type={this.state.total_fees_type[item]}
                />
              );
            })}
          {this.state.total_fees_type && (
        ...
        <div className="container-fluid mt--6">
          {this.state.data_updated && (
            ...
            >
              ...
              <span className="alert-text">
                <strong>Success!</strong> {this.state.data_updated}
              </span>
              <button
                type="button"
                className="close"
          ...
          <button className="btn btn-primary" onClick={(e) => this.onSubmit(e)}>
            {this.state.add_button_text ? (
              <span>{this.state.add_button_text}</span>
            ) : (
              <span>Save</span>
            )}
          </button>
